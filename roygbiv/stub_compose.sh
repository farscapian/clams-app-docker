...
for (( CLN_ID=0; CLN_ID<CLN_COUNT; CLN_ID++ )); do
    ...
    CLN_COMMAND="sh -c \"chown 1000:1000 /opt/c-lightning-rest/certs && service tor start && sleep 3 && lightningd --alias=${CLN_ALIAS} --proxy=127.0.0.1:9050 --bind-addr=0.0.0.0:9735 --announce-addr=${CLN_NAME}:9735 --announce-addr=${DOMAIN_NAME}:${CLN_PTP_PORT} --bitcoin-rpcuser=${BITCOIND_RPC_USERNAME} --bitcoin-rpcpassword=${BITCOIND_RPC_PASSWORD} --bitcoin-rpcconnect=bitcoind --bitcoin-rpcport=\${BITCOIND_RPC_PORT:-18443} --log-level=debug --dev-bitcoind-poll=20 --experimental-websocket-port=9736 --plugin=/opt/c-lightning-rest/plugin.js --plugin=/plugins/prism-plugin.py --experimental-offers"
